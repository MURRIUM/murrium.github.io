...
function initPhysics() {
    ...
    wheel = new Wheel(wheelX, wheelY, wheelRadius, decksQuantity, 0.15, 10.5);
...
Wheel.prototype = {
    createBody:function() {
        this.body = new p2.Body({mass:4, position:[this.x, this.y]});       // -- Adjust wheel mass if desired
    ...
    draw:function() {
        ...
        ctx.beginPath();
        ctx.fillStyle = '#DB9E36';    // Rim color
        ...
        var colors = ['#730046','#730046','#730046',
                     '#BFBB11','#BFBB11',
                     '#C93C00','#C93C00',
                     '#FFC200','#FFC200',
                     '#018EE8','#018EE8',
        ...
        for (var i = 0; i < this.segments; i++) {
          ...
          ctx.fillStyle = colors[randoms[i]];
            ...
            ctx.save();
            ctx.translate(0, 0);
            ctx.rotate((i+0.5)*Math.PI/(this.segments/2));
            ctx.fillText(member[randoms[i]],this.pRadius/5*Math.cos((0+0.1)*this.deltaPI), this.pRadius/1.7*Math.sin((0+0.1)*this.deltaPI));
            ctx.restore();
        }
